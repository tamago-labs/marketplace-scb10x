...
const useOrder = () => {
  ...
  const getAllOrders = useCallback(
    async ({ limit = 200, offset = 0, isMainnet }) => {
      ...
      const { data } = await axios.get(
        `${API_BASE}/orders?${chain}limit=${limit}&offset=${offset}`
      );
  ...
  const getTopSellers = useCallback(async () => {
    const { data } = await axios.get(
      `${API_BASE}/users?chain=42,97,80001,43113,137,56,43114,1`
    );
    const { users } = data;
    return users;
  }, []);
  ...
  const claimSeller = useCallback(
    async (order, pairChainId) => {
      ...
      if (
        NFT_MARKETPLACE.filter((item) => item.chainId === pairChainId)
          .length === 0
      ) {
        throw new Error("Marketplace contract is not available on given chain");
      }
  ...
  const getCollectionsTotal = useCallback(async () => {
    const { data } = await axios.get(
      `${API_BASE}/collections?chain=42,97,80001,43113,137,56,43114,1`
    );
    const { totalCount } = data;
    return totalCount;
  }, []);
