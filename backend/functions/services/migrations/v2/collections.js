...
const importFrontEndV2CollectionsToDB = async () => {
  try {
    for (let item of COLLECTIONS) {

      //checking if item already existed in the database
      ...
      if (existingDocument.empty) {
        ...
        console.log(`Already existed : ID ${item.chainId}.${item.assetAddress} `)
      }
